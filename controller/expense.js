...
const deleteexpense = async (req, res) => {
    ...
    try 
    {
        ...
        const record = await sequelize.query(`SELECT expenseamount FROM expenses WHERE id = ${expenseid} `, { type: QueryTypes.SELECT });
        const delamount = record[0].expenseamount;
        ...
        const totalExpense = Number(req.user.totalExpenses) - Number(delamount);
